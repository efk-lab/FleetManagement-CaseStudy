...
public class RestExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleMethodArgumentTypeMismatch(AccessDeniedException ex, WebRequest request) {
		...
		RestApiError apiError = buildRestApiError(ex, HttpStatus.UNAUTHORIZED);
		
		return buildResponseEntity(apiError);

	}

	@ExceptionHandler(FleetManagerException.class)
	protected ResponseEntity<Object> handleFleetManagerException(FleetManagerException ex, WebRequest request) {

		RestApiError apiError = buildRestApiError(ex, HttpStatus.UNPROCESSABLE_ENTITY);
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, HttpHeaders headers, HttpStatus status, WebRequest request) {
		...
		RestApiError apiError = buildRestApiError(ex, HttpStatus.BAD_REQUEST);
		
		return buildResponseEntity(apiError);

	}
	
	private RestApiError buildRestApiError(Exception ex, HttpStatus httpStatus) {
		
		RestApiError apiError = new RestApiError(httpStatus);
